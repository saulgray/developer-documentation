...
#### 6. How do I pass in panelist profile data into Fulcrum and get it back?

Data can be passed to Fulcrum by appending variables to Supplier Entry Links, and received back on the Supplier Redirect Links. The latter can be set up either through default Supplier Redirect URLs, or on a per survey basis on [Create a Link](#post-create-a-link) or [Update a Link](#put-update-a-link) calls. [Here](http://) is a list of variables that can be passed into Fulcrum upon a respondent’s entry, as well as received by the Supplier upon the respondent’s return.  Fulcrum variables are added as [%VARIABLE%] where "VARIABLE" is the information which is attached and "[%     %]" notifies Fulcrum that it needs to assign a variable at this part of the URL. We highly recommend including the top 20 qualifications for the survey study in the redirect link. If you are missing some demographic data about a respondent, you can request Fulcrum to pass it back to you. Fulcrum stores demographic data associated with a respondent for 30 days.

You can use any label for the variables that you want, but within the "[%%]" you must use the exact name that Fulcrum uses for the variable.  For respondent data variables the name of the Fulcrum Standard question is used for this.  For example, if you are looking to collect respondent's gender on the redirect, you will find the question asking gender in our Questions Library (http://developer.lucidhq.com/#question-library) and use the EXACT name listed for that question within the "[%%]" so it would be [%GENDER%].  Note that these variables are case sensitive.

For sending in single punch question data into Fulcrum, you can append the data to the end of the entry link, for instance as &AGE=37. For multipunch questions, each answer will have to be appended separately, for instance &STANDARD_PETS=1&STANDARD_PETS=3. Note the special code -3105 indicating “None of the above” for multipunch questions, which can be passed in as &STANDARD_PETS=-3105.

Receiving data back from Fulcrum is done in the same way for single punch questions, for instance age of a respondent would be passed back as &AGE=37. This is not the case for multipunch questions, where Fulcrum will return all the answers for the same parameter, for example &STANDARD_PETS=10100000000.
